...
public class PowerPos : MonoBehaviour
{
    ...
    Collider Col;
    Swipetool st = new Swipetool();
    public static bool cldid = false;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            if (Swipetool.pwr == false)
            {
                ...
                //st.powerUps();
                //gameObject.AddComponent<Swipetool>().powerUps();
                PowerUpTut.powertutdone = true; 
                cldid = true;
